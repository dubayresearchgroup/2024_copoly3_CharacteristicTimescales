...
def rdf(filename,cutoff,bin_width):
...
def rdf_pairAA(filename,cutoff,bin_width):
    id2pos, id2type = gettraj(filename)
    bondpairs = getbond2(getbond1(filename), id2type)
    bins = Bins(cutoff)
    bins.make(id2pos, id2type)
...
def rdf_pairBB(filename,cutoff,bin_width):
    id2pos, id2type = gettraj(filename)
    bondpairs = getbond2(getbond1(filename), id2type)
    bins = Bins(cutoff)
    bins.make(id2pos, id2type)
...
def rdf_pairAB(filename,cutoff,bin_width):
    id2pos, id2type = gettraj(filename)
    bondpairs = getbond2(getbond1(filename), id2type)
    bins = Bins(cutoff)
    bins.make(id2pos, id2type)
...
def calc(simname, step = rdf_step, cutoff = rdf_cutoff, bin_width = BIN_WIDTH):
    fetch(simname+'.lammpstrj',step,simname+'_{}snap.lammpstrj'.format(step))
    fetch(simname+'-bond',step,simname+'_{}snap-bond'.format(step))
    try:
        sb.call('ghead -n -1 '+simname+'_{}snap.lammpstrj'.format(step)+' > tmp && mv tmp '+simname+'_{}snap.lammpstrj'.format(step),shell=True)
        sb.call('ghead -n -1 '+simname+'_{}snap-bond'.format(step)+' > tmp && mv tmp '+simname+'_{}snap-bond'.format(step),shell=True)
    ...
    x, rdf_all = rdf(simname,cutoff,bin_width)
    x, rdf_AA = rdf_pairAA(simname,cutoff,bin_width)
    x, rdf_AB = rdf_pairAB(simname,cutoff,bin_width)
    x, rdf_BB = rdf_pairBB(simname,cutoff,bin_width)
    RDFs = [rdf_step, x, rdf_all, rdf_AA, rdf_BB, rdf_AB]
    with open(simname+'_{}snap_RDFs.pickle'.format(step),'wb') as f:
        pickle.dump(RDFs,f)
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print('Usage: rdf_script [FLAGS] filename. No extensions on filename, and filename of trajectory and bond file must match. \n')
        print('Possible flags: -step step to take RDF snapshot, -cut cutoff distance for RDF calculation, -bin bin size for RDF calculation, -box simulation box size. \n')
        exit()
    i = 1
    while i < len(sys.argv):
        if sys.argv[i]=='-step':
            rdf_step = int(sys.argv[i+1])
            i += 2
            continue
        if sys.argv[i]=='-cut':
            rdf_cutoff = int(sys.argv[i+1])
            i += 2
            continue    
        if sys.argv[i]=='-bin':
