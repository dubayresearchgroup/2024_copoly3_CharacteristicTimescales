...
Dump::~Dump()
{
  ...
  if (if_flag) {
    memory->destroy(if_string);
    memory->destroy(if_copy);
    memory->destroy(if_work);
  }
...
void Dump::write()
{
  ...
  if (if_flag) {
    strcpy(if_copy,if_string);
    input->substitute(if_copy,if_work,if_maxcopy,if_maxwork,0);
    //printf("if_copy: %s\n", if_copy);
    if (input->variable->evaluate_boolean(if_copy) == 0.0){
      return;
    }
  }
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"append") == 0) {
      ...
      *ptr = '%';
      ...
      if_string = new char[n];
      strcpy(if_string,arg[iarg+1]);
      if_copy = (char *) memory->smalloc(n*sizeof(char),"dump:if_copy");
      if_work = (char *) memory->smalloc(n*sizeof(char),"dump:if_work");
      if_maxcopy = if_maxwork = n;
